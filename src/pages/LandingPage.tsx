...
const ALL_EMOJI = emoji.all()
const EMOJI_LIST = ALL_EMOJI.map((emoji: emoji.Emoji) => {
  return {
    name: emoji.name,
...
/** Available groups in which emoji are categorized */
const GROUP_LIST = [
  ...new Set(
    EMOJI_LIST.reduce((acc: string[], next) => {
      return [next.group, ...acc]
    }, []),
  ),
] as const

/** Available subGroups in which emoji are categorized */
const SUB_GROUP_LIST = [
  ...new Set(
    EMOJI_LIST.reduce((acc: string[], next) => {
      return [next.subGroup, ...acc]
    }, []),
  ),
] as const
...
type EmojiList = typeof EMOJI_LIST
...
function EmojiForm() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl p-3 md:p-8 flex flex-col gap-3">
      ...
      {searchStr ? <EmojiResults emojiList={EMOJI_LIST} searchStr={searchStr} /> : null}
